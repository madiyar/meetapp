import { setFreeRooms, setRooms, setLoading, setRoom } from "../actions/rooms.actions";
...
export function getFreeRooms(startDate, endDate) {
    return function(dispatch, getState) {
        ...
        fetch('http://localhost:8080/meetings/find-room', meetingsOptions)
...
export function getRooms() {
    return function(dispatch, getState) {
        dispatch(setLoading(true));
        fetch('http://localhost:8080/meetings/today')
            .then(response => response.json())
            .then(meetings => {
                let todayMeetings = [];
                meetings.map(meeting => {
